...
function cloudClear(){try{localStorage.removeItem(cloudKey())}catch{}cloudPending=false;cloudBase=cloudCopy(state);cloudStatus('Salvo no Firebase');}
...
for(const name of ['saveProduct','delProduct','saveInput','delInput','saveComp','delComp','saveLabor','resetData','saveProcess','deleteProcess','importBackup','importLegacy']){
  ...
  window[name]=async function(...args){
    ...
    catch(error){if(cloudDirty){cloudPending=true;cloudRemember()}cloudStatus(cloudPending?'Alterações pendentes — ainda não confirmadas no Firebase':'Falha ao salvar');cloudToast(error.message+' Exporte um backup para preservar os dados.');}
...
async function refreshCloud(){
  ...
  try{state=await ForgeServices.repository.load(ForgeServices.auth.current(),DEFAULT_STATE);cloudBase=cloudCopy(state);state.view='dashboard';state.selectedProduct=null;render();cloudStatus('Dados carregados do Firebase')}
...
async function cloudLogin(event){
  ...
  try{
    ...
    cloudStatus(cloudPending?'Cópia pendente recuperada — use Tentar salvar':pendingProblem?'Dados carregados; havia uma cópia local pendente inválida':'Dados carregados do Firebase');
    if(pendingProblem&&!cloudPending)cloudToast('Login concluído. A cópia local pendente não pôde ser restaurada; seus dados do Firebase foram carregados.');
...
cloudBar.innerHTML='<span id="cloudStatus" role="status" aria-live="polite">Entre para acessar o Firebase</span><button class="btn" id="cloudRetry" hidden>Tentar salvar</button><button class="btn" id="cloudRefresh">Atualizar dados</button><button class="btn" id="cloudExport">Exportar backup</button>';
...
window.addEventListener('beforeunload',event=>{if(cloudPending||cloudBusy){event.preventDefault();event.returnValue=''}});
  ...
  if(loginError)loginError.textContent=error?.message||'Não foi possível carregar a integração Firebase.';
});
